...
async function getHotItems(): Promise<HotItemsData> {
  ...
  return await ssrSafeFetch<HotItemsData>(url, {
    fallback: { items: [], lastUpdated: new Date().toISOString() },
    cache: 'no-store',
    headers: {
...
async function getPlatformStats() {
  ...
  return await ssrSafeFetch(url, {
    fallback: { totalThreads: 0, totalMembers: 0, totalPosts: 0, activeToday: 0 },
    cache: 'no-store',
    headers: {
      'Content-Type': 'application/json',
    },
