...
export default function DiscussionsClient({ initialThreads }: DiscussionsClientProps) {
  ...
  const handleNewThread = () => {
    console.log("[NewThread] Button clicked. Auth status:", isAuthenticated);
    
    // For authenticated users, navigate directly
    if (isAuthenticated) {
      console.log("[NewThread] User authenticated, navigating directly to /discussions/new");
      router.push('/discussions/new');
      return;
    }
    
    // For unauthenticated users, use requireAuth with proper callback
    console.log("[NewThread] User not authenticated, showing auth prompt");
    requireAuth((authSuccess: boolean = true) => {
      console.log("[NewThread] Auth callback fired. Success:", authSuccess);
      
      if (authSuccess) {
        // Double-check authentication state and navigate
        console.log("[NewThread] Auth successful, attempting navigation");
        // Use a small timeout to ensure auth state propagates
        setTimeout(() => {
          console.log("[NewThread] Executing navigation to /discussions/new");
          router.push('/discussions/new');
        }, 200);
      } else {
        console.log("[NewThread] Auth was not successful or cancelled");
      }
    });
  };
