...
export default function AdminUsers() {
  ...
  const bulkBanMutation = useMutation({
    mutationFn: async ({ userIds, reason }: { userIds: number[]; reason: string }) => {
      return apiRequest("POST", "/api/admin/users/bulk/ban", { userIds, reason });
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/users"] });
      toast({ title: `Successfully banned ${data.affectedUsers} users` });
      setSelectedUserIds(new Set());
      setBulkActionOpen(false);
    },
    onError: () => {
      toast({ title: "Failed to ban users", variant: "destructive" });
    }
  });
  ...
  const bulkActivateMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/users"] });
      toast({ title: `Successfully activated ${data.affectedUsers} users` });
      setSelectedUserIds(new Set());
      setBulkActionOpen(false);
    },
    onError: () => {
      toast({ title: "Failed to activate users", variant: "destructive" });
    }
  });
  
  const bulkRoleMutation = useMutation({
    mutationFn: async ({ userIds, role }: { userIds: number[]; role: string }) => {
      return apiRequest("POST", "/api/admin/users/bulk/role", { userIds, role });
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/users"] });
      toast({ title: `Successfully updated role for ${data.affectedUsers} users` });
      setSelectedUserIds(new Set());
      setBulkActionOpen(false);
    },
    onError: () => {
