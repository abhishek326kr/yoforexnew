...
const corsOptions = {
  origin: function (origin: string | undefined, callback: (err: Error | null, allow?: boolean) => void) {
    ...
    const replitDomainPatterns = [
      /^https?:\/\/.*\.replit\.app(:\d+)?$/,      // Replit app domains (multi-level, optional port)
      /^https?:\/\/.*\.replit\.dev(:\d+)?$/,      // Replit dev domains (multi-level, optional port)
      /^https?:\/\/.*\.repl\.co(:\d+)?$/,         // Replit co domains (multi-level, optional port)
      /^https?:\/\/.*\.repl\.run(:\d+)?$/,        // Replit run domains (multi-level, optional port)
    ];
