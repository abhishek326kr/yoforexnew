...
async function createEmailTemplate(
  ...
  // Use BASE_URL with fallback to prevent broken links
  const baseUrl = process.env.BASE_URL || 'https://yoforex.net';
    ...
    <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f3f4f6;">
      <div style="max-width: 600px; margin: 0 auto; background: white;">
        ...
        <div style="background: #f9fafb; padding: 24px; text-align: center; border-top: 1px solid #e5e7eb;">
          ...
          <p style="margin: 0;">
            <a href="${baseUrl}/settings/notifications" style="color: #2563eb; font-size: 12px; text-decoration: none;">Email Preferences</a>
            <span style="color: #d1d5db; margin: 0 8px;">|</span>
            <a href="${baseUrl}/unsubscribe" style="color: #2563eb; font-size: 12px; text-decoration: none;">Unsubscribe</a>
          </p>
        </div>
      </div>
    </body>
...
export const emailService = {
  ...
  async sendFilePurchaseConfirmation(
    ...
    const content = `
      ...
      <p style="color: #6b7280; font-size: 14px; margin: 16px 0 0 0;">
        💡 <strong>Tip:</strong> Your download link will remain active. You can also access your purchased files anytime from your <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/dashboard" style="color: #2563eb;">Dashboard</a> under the Purchases tab.
      </p>
    `;
  ...
  async sendFileSaleNotification(
    ...
    const content = `
      ...
      <div style="text-align: center; margin: 24px 0;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/dashboard" style="display: inline-block; background: #16a34a; color: white; padding: 12px 28px; text-decoration: none; border-radius: 8px; font-weight: bold;">
  ...
  async sendCommentNotification(to: string, commenterName: string, threadTitle: string, commentPreview: string, threadSlug: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/threads/${threadSlug}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: 8px;">View Thread</a>
    `;
  ...
  async sendLikeNotification(to: string, likerName: string, contentType: string, contentTitle: string, contentUrl: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${contentUrl}" style="display: inline-block; background: #ec4899; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View ${safeContentType}</a>
    `;
  ...
  async sendFollowNotification(to: string, followerName: string, followerUsername: string, followerAvatar?: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/users/${safeFollowerUsername}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: 8px;">View Profile</a>
    `;
  ...
  async sendWithdrawalRequestReceived(to: string, amount: number, method: string, requestId: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/wallet/withdrawals/${safeRequestId}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Track Status</a>
    `;
  ...
  async sendCoinsReceived(to: string, amount: number, source: string, newBalance: number): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/wallet" style="display: inline-block; background: #f59e0b; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Wallet</a>
    `;
  ...
  async sendProductSold(to: string, productName: string, buyerName: string, price: number, earnings: number): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/dashboard" style="display: inline-block; background: #10b981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Dashboard</a>
    `;
  ...
  async sendProductPublished(to: string, productName: string, productSlug: string, category: string): Promise<void> {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/content/${safeProductSlug}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">View Listing</a>
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/content/${safeProductSlug}/edit" style="display: inline-block; background: white; border: 2px solid #2563eb; color: #2563eb; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">Edit</a>
      </div>
    `;
  ...
  async sendPasswordReset(to: string, resetToken: string, expiresIn: string = '1 hour'): Promise<void> {
    const resetUrl = `${(process.env.BASE_URL || 'https://yoforex.net')}/reset-password?token=${resetToken}`;
  ...
  async sendUsernameChanged(to: string, oldUsername: string, newUsername: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/settings" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Settings</a>
    `;
  ...
  async sendNewMessage(to: string, senderName: string, senderUsername: string, messagePreview: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/messages" style="display: inline-block; background: #8b5cf6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Read Message</a>
    `;
  ...
  async sendLevelUp(to: string, newLevel: string, xp: number, rewards: string[]): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/dashboard" style="display: inline-block; background: #f59e0b; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Dashboard</a>
    `;
  ...
  async sendLeaderboardRank(to: string, rank: number, category: string, points: number): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/leaderboard" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Leaderboard</a>
    `;
  ...
  async sendEmailVerification(to: string, username: string, verificationToken: string): Promise<void> {
    ...
    const verifyUrl = `${(process.env.BASE_URL || 'https://yoforex.net')}/verify-email?token=${verificationToken}`;
  ...
  async sendPurchaseReceipt(to: string, productName: string, price: number, purchaseId: string, downloadUrl: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/purchases" style="display: inline-block; background: white; border: 2px solid #2563eb; color: #2563eb; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">View All Purchases</a>
    `;
  ...
  async sendThreadReply(to: string, replierName: string, threadTitle: string, replyPreview: string, threadSlug: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/threads/${threadSlug}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Reply</a>
    `;
  ...
  async sendContentShared(to: string, sharerName: string, contentTitle: string, contentUrl: string, platform?: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${contentUrl}" style="display: inline-block; background: #0ea5e9; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Content</a>
    `;
  ...
  async sendUnfollowNotification(to: string, unfollowerName: string, unfollowerUsername: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/dashboard" style="display: inline-block; background: #6b7280; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Dashboard</a>
    `;
  ...
  async sendFriendRequest(to: string, requesterName: string, requesterUsername: string, message?: string): Promise<void> {
    ...
    const content = `
      ...
      ` : ''}
      <div style="margin-top: 16px;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/requests" style="display: inline-block; background: #10b981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">Accept</a>
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/users/${safeRequesterUsername}" style="display: inline-block; background: white; border: 2px solid #2563eb; color: #2563eb; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Profile</a>
      </div>
    `;
  ...
  async sendMentionNotification(to: string, mentionerName: string, contentType: string, contentTitle: string, contentUrl: string, context?: string): Promise<void> {
    ...
    const content = `
      ...
      ` : ''}
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${contentUrl}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Mention</a>
    `;
  ...
  async sendLowBalanceWarning(to: string, currentBalance: number, threshold: number = 10): Promise<void> {
    const content = `
      ...
      <div style="margin-top: 16px;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/recharge" style="display: inline-block; background: #f59e0b; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">Recharge Now</a>
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/earn-coins" style="display: inline-block; background: white; border: 2px solid #2563eb; color: #2563eb; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">Earn Coins</a>
      </div>
    `;
  ...
  async sendPostApproved(to: string, postTitle: string, postUrl: string, category: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${postUrl}" style="display: inline-block; background: #10b981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Post</a>
    `;
  ...
  async sendPostRejected(to: string, postTitle: string, reason: string, guidelines?: string): Promise<void> {
    ...
    const content = `
      ...
      ` : ''}
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/publish" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Edit and Resubmit</a>
    `;
  ...
  async sendCommentSuccess(to: string, threadTitle: string, threadUrl: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${threadUrl}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Thread</a>
    `;
  ...
  async sendFirstPostMilestone(to: string, postTitle: string, postUrl: string, tips: string[]): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${postUrl}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Your Post</a>
    `;
  ...
  async sendUploadSuccess(to: string, fileName: string, fileType: string, fileSize: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/uploads" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Uploads</a>
    `;
  ...
  async sendPostPopular(to: string, postTitle: string, postUrl: string, views: number, threshold: number): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${postUrl}" style="display: inline-block; background: #ef4444; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Post</a>
    `;
  ...
  async sendBestAnswer(to: string, threadTitle: string, threadUrl: string, rewardCoins: number): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${threadUrl}" style="display: inline-block; background: #f59e0b; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Thread</a>
    `;
  ...
  async sendQuoteNotification(to: string, quoterName: string, originalPost: string, threadTitle: string, threadUrl: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${threadUrl}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Quote</a>
    `;
  ...
  async sendWeeklyActivitySummary(to: string, username: string, stats: {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/dashboard" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: 8px;">View Full Dashboard</a>
    `;
  ...
  async sendProductListed(to: string, productName: string, productSlug: string, category: string, price: number): Promise<void> {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/marketplace/${safeProductSlug}" style="display: inline-block; background: #10b981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">View Listing</a>
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/marketplace/${safeProductSlug}/edit" style="display: inline-block; background: white; border: 2px solid #2563eb; color: #2563eb; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">Edit Product</a>
      </div>
    `;
  ...
  async sendProductReview(to: string, productName: string, reviewerName: string, rating: number, review: string, productUrl: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${productUrl}#reviews" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Review</a>
    `;
  ...
  async sendSalePendingPayment(to: string, productName: string, buyerName: string, amount: number, orderId: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/orders/${safeOrderId}" style="display: inline-block; background: #f59e0b; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Order Details</a>
    `;
  ...
  async sendPayoutProcessed(to: string, amount: string, method: string, transactionId: string, processingTime: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/payouts/${safeTransactionId}" style="display: inline-block; background: #10b981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Details</a>
    `;
  ...
  async sendRefundIssued(to: string, productName: string, amount: number, reason: string, refundId: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/wallet" style="display: inline-block; background: #3b82f6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Wallet</a>
    `;
  ...
  async sendNewDeviceLogin(to: string, deviceInfo: {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/settings/security" style="display: inline-block; background: #dc2626; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">Secure Account</a>
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/settings/sessions" style="display: inline-block; background: white; border: 2px solid #6b7280; color: #6b7280; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">View All Sessions</a>
      </div>
    `;
  ...
  async sendAccountWarning(to: string, warningType: 'warning' | 'suspension', reason: string, duration?: string, appealUrl?: string): Promise<void> {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/guidelines" style="display: inline-block; background: #6b7280; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">View Guidelines</a>
  ...
  async sendContentReported(to: string, contentTitle: string, contentType: string, reportReason: string, reportId: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/reports/${safeReportId}" style="display: inline-block; background: #6b7280; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Report Details</a>
    `;
  ...
  async sendContentRemoved(to: string, contentTitle: string, contentType: string, reason: string, violatedPolicy: string): Promise<void> {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/guidelines" style="display: inline-block; background: #6b7280; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">View Guidelines</a>
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/support/appeal" style="display: inline-block; background: white; border: 2px solid #2563eb; color: #2563eb; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">Appeal</a>
      </div>
    `;
  ...
  async sendWeeklyDigest(to: string, username: string, stats: {
    ...
    const hotDiscussionsHtml = stats.hotDiscussions.map(thread => `
      <li style="margin: 8px 0;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/threads/${thread.slug}" style="color: #2563eb; text-decoration: none; font-weight: bold;">
    ...
    const topContentHtml = stats.topContent.map(content => `
      <li style="margin: 8px 0;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/content/${content.slug}" style="color: #2563eb; text-decoration: none; font-weight: bold;">
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: 8px;">Visit YoForex</a>
    `;
  ...
  async sendSubscriptionAutoRenewed(to: string, planName: string, amount: string, nextRenewalDate: string, invoiceUrl?: string): Promise<void> {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        ...
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/settings/subscription" style="display: inline-block; background: white; border: 2px solid #6b7280; color: #6b7280; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">Manage Subscription</a>
      </div>
    `;
  ...
  async sendSubscriptionCanceled(to: string, planName: string, endDate: string, reason?: string): Promise<void> {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/pricing" style="display: inline-block; background: #10b981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">Reactivate</a>
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/feedback" style="display: inline-block; background: white; border: 2px solid #6b7280; color: #6b7280; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">Give Feedback</a>
      </div>
    `;
  ...
  async sendContestEntry(to: string, contestName: string, entryId: string, submissionTitle: string, deadline: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/contests/${safeEntryId}" style="display: inline-block; background: #0ea5e9; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Entry</a>
    `;
  ...
  async sendBadgeEarned(to: string, badgeName: string, badgeIcon: string, badgeDescription: string, totalBadges: number): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/profile/badges" style="display: inline-block; background: #7c3aed; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View All Badges</a>
    `;
  ...
  async sendGroupInvite(to: string, groupName: string, inviterName: string, memberCount: number, groupDescription: string): Promise<void> {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/groups/join/${encodeURIComponent(groupName)}" style="display: inline-block; background: #10b981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">Accept Invite</a>
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/groups/${encodeURIComponent(groupName)}" style="display: inline-block; background: white; border: 2px solid #6b7280; color: #6b7280; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Group</a>
      </div>
    `;
  ...
  async sendEventReminder(to: string, eventName: string, eventDate: string, eventTime: string, eventType: string, joinUrl: string): Promise<void> {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        ...
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/events/calendar" style="display: inline-block; background: white; border: 2px solid #6b7280; color: #6b7280; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">Add to Calendar</a>
      </div>
    `;
  ...
  async sendRecommendedPosts(to: string, username: string, posts: Array<{
    ...
    const postsHtml = posts.map(post => `
      <div style="background: #f9fafb; border: 1px solid #e5e7eb; padding: 16px; border-radius: 8px; margin-bottom: 12px;">
        <h4 style="margin: 0 0 8px 0;">
          <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/posts/${post.slug}" style="color: #2563eb; text-decoration: none; font-weight: bold;">
    ...
    const content = `
      ...
      ${postsHtml}
      <div style="margin-top: 16px; text-align: center;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/discover" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Discover More</a>
      </div>
    `;
  ...
  async sendDownloadLimitReached(to: string, currentLimit: number, downloadsToday: number, resetTime: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/upgrade" style="display: inline-block; background: #f59e0b; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Upgrade to Premium</a>
    `;
  ...
  async sendFileExpiring(to: string, fileName: string, expiryDate: string, fileSize: string, downloadUrl: string): Promise<void> {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        <a href="${downloadUrl}" style="display: inline-block; background: #10b981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">Download Now</a>
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/files/extend" style="display: inline-block; background: white; border: 2px solid #2563eb; color: #2563eb; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">Extend Expiry</a>
      </div>
    `;
  ...
  async sendInactiveUserReengagement(to: string, username: string, daysSinceLastVisit: number, newFeatures: string[], popularContent: Array<{title: string; slug: string}>): Promise<void> {
    ...
    const contentHtml = popularContent.map(item => `
      <li style="margin: 4px 0;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/content/${item.slug}" style="color: #2563eb; text-decoration: none;">
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Return to YoForex</a>
    `;
  ...
  async sendBirthdayWishes(to: string, username: string, bonusCoins: number, specialOffer?: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/wallet" style="display: inline-block; background: #ec4899; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Claim Birthday Gift</a>
    `;
  ...
  async sendAnniversaryCelebration(to: string, username: string, years: number, rewardCoins: number, achievements: string[]): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/profile/achievements" style="display: inline-block; background: #7c3aed; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Achievements</a>
    `;
  ...
  async sendReferralBonus(to: string, referredUsername: string, bonusCoins: number, totalReferrals: number, nextMilestone?: {referrals: number; bonus: number}): Promise<void> {
    ...
    const content = `
      ...
      <div style="margin-top: 16px;">
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/referrals" style="display: inline-block; background: #10b981; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-right: 12px;">View Referrals</a>
        <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/referrals/share" style="display: inline-block; background: white; border: 2px solid #2563eb; color: #2563eb; padding: 10px 22px; text-decoration: none; border-radius: 8px; font-weight: bold;">Share Link</a>
      </div>
    `;
  ...
  async sendMaintenanceNotice(to: string, maintenanceDate: string, startTime: string, duration: string, affectedServices: string[], reason: string): Promise<void> {
    ...
    const content = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/status" style="display: inline-block; background: #6b7280; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Check Status</a>
    `;
  ...
  async sendAbandonmentEmail(to: string, subject: string, content: string): Promise<void> {
    ...
    const htmlContent = `
      ...
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/dashboard" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: 8px;">Return to Dashboard</a>
    `;
  ...
  // Queue methods for notification system
  async queueNotificationEmail(userId: string, notification: any, options?: { priority?: 'high' | 'medium' | 'low' }): Promise<void> {
    try {
      // Get user email
      const [user] = await db.select()
        .from(users)
      ...
      const to = user.email;
      const subject = notification.title || 'YoForex Notification';
      const content = `
        <h2 style="color: #111827; margin: 0 0 16px 0; font-size: 24px;">${escapeHtml(notification.title || 'Notification')}</h2>
        <p style="color: #374151; font-size: 16px; line-height: 1.5; margin: 0 0 16px 0;">
          ${escapeHtml(notification.message || '')}
        </p>
        ${notification.actionUrl ? `
          <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${notification.actionUrl}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Details</a>
        ` : ''}
      `;
      ...
      // Queue the email (immediately send for now)
      await transporter.sendMail({
        from: `"${process.env.SMTP_FROM_NAME || 'YoForex'}" <${process.env.SMTP_FROM_EMAIL}>`,
        to,
        subject,
      ...
      console.error('[EmailService] Failed to queue notification email:', error);
    }
  },

  async queueDigestNotification(userId: string, notification: any): Promise<void> {
    // Store notification for later digest (for now just log it)
    console.log(`[EmailService] Queuing notification for digest: user=${userId}, type=${notification.type}`);
    // In a real implementation, this would store notifications in a queue table
    // for batch processing later via sendDailyDigest/sendWeeklyDigest
  },

  async sendDailyDigest(email: string, username: string, notifications: any[]): Promise<void> {
    if (!email || notifications.length === 0) return;

    const safeUsername = escapeHtml(username);
    const notificationsHtml = notifications.slice(0, 10).map(n => `
      <div style="background: #f9fafb; padding: 12px; border-radius: 8px; margin: 8px 0;">
        <p style="margin: 0 0 4px 0; color: #111827; font-weight: 600;">${escapeHtml(n.title || 'Notification')}</p>
        <p style="margin: 0; color: #6b7280; font-size: 14px;">${escapeHtml(n.message || '')}</p>
        ${n.actionUrl ? `
          <a href="${(process.env.BASE_URL || 'https://yoforex.net')}${n.actionUrl}" style="color: #2563eb; font-size: 14px; text-decoration: none;">View →</a>
        ` : ''}
      </div>
    `).join('');

    const content = `
      <h2 style="color: #111827; margin: 0 0 16px 0; font-size: 24px;">📨 Your Daily Digest</h2>
      <p style="color: #374151; font-size: 16px; line-height: 1.5; margin: 0 0 16px 0;">
        Hi <strong>${safeUsername}</strong>, here's what you missed today:
      </p>
      <div style="margin: 16px 0;">
        ${notificationsHtml}
      </div>
      ...
      ` : ''}
      <a href="${(process.env.BASE_URL || 'https://yoforex.net')}/notifications" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: 8px;">View All Notifications</a>
    `;
