    ...
    try {
      ...
      const content = `
        ...
        ${notification.actionUrl ? `
          <a href="${process.env.BASE_URL}${notification.actionUrl}" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">View Details</a>
        ` : ''}
      `;
  ...
  async sendDailyDigest(email: string, username: string, notifications: any[]): Promise<void> {
    ...
    const notificationsHtml = notifications.slice(0, 10).map(n => `
      <div style="background: #f9fafb; padding: 12px; border-radius: 8px; margin: 8px 0;">
        ...
        ${n.actionUrl ? `
          <a href="${process.env.BASE_URL}${n.actionUrl}" style="color: #2563eb; font-size: 14px; text-decoration: none;">View →</a>
        ` : ''}
      </div>
    `).join('');
    ...
    const content = `
      ...
      ` : ''}
      <a href="${process.env.BASE_URL}/notifications" style="display: inline-block; background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: 8px;">View All Notifications</a>
    `;
