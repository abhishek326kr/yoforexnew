...
export async function scanNewThreads(minutesAgo: number = 30) {
  ...
  // Fix: Also include threads with NULL createdAt (defaultNow might not work for existing records)
  const newThreads = await db.select()
    .from(forumThreads)
    .where(
      or(
        gte(forumThreads.createdAt, cutoffTime),
        isNull(forumThreads.createdAt)
      )
    )
    .orderBy(desc(forumThreads.createdAt))
    .limit(20); // Add limit to prevent overwhelming the bot system
  
  console.log(`[BOT ENGINE] Found ${newThreads.length} threads to process (last ${minutesAgo} minutes or without timestamp)`);
...
export async function scanNewContent(minutesAgo: number = 30) {
  ...
  // Fix: Handle NULL createdAt and scan all approved content for bots to interact with
  ...
  console.log(`[BOT ENGINE] Found ${newContent.length} content items to process (affordable and approved)`);
...
export async function runBotEngine() {
  ...
  // Use default aggression level of 5 (configurable from settings in the future)
  const defaultAggressionLevel = 5;
  for (const thread of newThreads) {
    await executeLikes(thread.id, defaultAggressionLevel);
