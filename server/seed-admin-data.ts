...
export async function seedAdminData() {
  ...
  try {
    ...
    const categories: ('technical' | 'billing' | 'account' | 'general' | 'other')[] = ['technical', 'billing', 'account', 'general', 'other'];
    const tickets = [];
    ...
    await db.insert(featureFlags).values([
      {
        slug: 'new_dashboard_ui',
        scope: 'feature',
        targetPath: '/dashboard',
        status: 'enabled',
        rolloutType: 'percentage',
        rolloutConfig: { percentage: 50, users: [adminIds[0], adminIds[1]] },
        seoTitle: 'New Dashboard UI',
        seoDescription: 'Enable redesigned dashboard interface',
        createdAt: new Date(Date.now() - 15 * 24 * 60 * 60 * 1000),
        updatedAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000),
      },
      {
        slug: 'ai_content_moderation',
        scope: 'feature',
        targetPath: '/moderation',
        status: 'enabled',
        rolloutType: 'percentage',
        rolloutConfig: { percentage: 100 },
        seoTitle: 'AI Content Moderation',
        seoDescription: 'Use AI to detect spam and inappropriate content',
        createdAt: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
      ...
      },
      {
        ...
        status: 'disabled',
        rolloutType: 'users',
        rolloutConfig: { users: [adminIds[0]] },
        seoTitle: 'Social Media Sharing',
        seoDescription: 'Allow users to share content on social media',
      ...
      },
      {
        ...
        rolloutConfig: { percentage: 100 },
        seoTitle: 'Dark Mode',
        seoDescription: 'Enable dark theme option',
        createdAt: new Date(Date.now() - 45 * 24 * 60 * 60 * 1000),
      ...
      },
      {
        ...
        seoDescription: 'Enable referral rewards system',
        createdAt: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000),
    ...
    const events = [];
    const securityEventTypes: ('login_failed' | 'api_rate_limit' | 'suspicious_ip' | 'login_bruteforce' | 'api_abuse')[] = ['login_failed', 'suspicious_ip', 'login_bruteforce', 'api_abuse', 'api_rate_limit'];
    for (let i = 0; i < 30; i++) {
      const isResolved = i >= 20;
      events.push({
        type: securityEventTypes[i % securityEventTypes.length],
        severity: severities[i % severities.length] as 'low' | 'medium' | 'high',
        userId: i % 5 === 0 ? null : allUserIds[i % allUserIds.length],
        ipAddress: `${Math.floor(Math.random() * 256)}.${Math.floor(Math.random() * 256)}.${Math.floor(Math.random() * 256)}.${Math.floor(Math.random() * 256)}`,
        description: `Security event: ${securityEventTypes[i % securityEventTypes.length]} detected with ${Math.floor(Math.random() * 10) + 1} attempts`,
