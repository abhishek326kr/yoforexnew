...
/**
 * Fix Onboarding Coins Script - Simple Version
 * 
 * This script retroactively awards coins to users who completed onboarding tasks
 * but didn't receive their coins. Uses simple SQL without complex constraints.
 * 
 * Usage:
 *   npm run tsx scripts/fix-onboarding-coins-simple.ts --dry-run  # Test mode
 *   npm run tsx scripts/fix-onboarding-coins-simple.ts            # Execute for real
 */
...
async function main() {
  console.log(`\n${CYAN}╔═══════════════════════════════════════════════════════╗${RESET}`);
  console.log(`${CYAN}║   Fix Onboarding Coins - Simple Direct SQL Version   ║${RESET}`);
  ...
  try {
        ...
        for (const task of tasksToAward) {
          ...
          if (!isDryRun) {
            try {
              ...
              // Check again right before insert to prevent race conditions
              ...
              const walletCheck = await db.execute(sql`
                SELECT COUNT(*) as count FROM user_wallet WHERE user_id = ${user.id}
              `);
              
              const hasWallet = (walletCheck.rows[0] as any).count > 0;
              
              if (hasWallet) {
                // Update existing wallet
                await db.execute(sql`
                  UPDATE user_wallet
                  SET balance = COALESCE(balance, 0) + ${task.coins},
                      available_balance = COALESCE(available_balance, 0) + ${task.coins},
                      updated_at = NOW()
                  WHERE user_id = ${user.id}
                `);
              } else {
              ...
              await db.execute(sql`
                INSERT INTO coin_journal_entries (
                  id,
                  transaction_id,
                  account_type,
                  account_id,
                ...
                VALUES (
                  ...
                  ${task.coins},
                  ${user.total_coins},
                  ${(user.total_coins as number) + task.coins},
                  ${task.description + ' - Transaction ' + transactionId},
                  ${JSON.stringify({ trigger: task.trigger, task: task.task, source: 'retroactive_fix' })}::jsonb,
                  NOW()
                )
              `);
    ...
    if (isDryRun) {
      ...
      console.log(`  ${CYAN}npm run tsx scripts/fix-onboarding-coins-simple.ts${RESET}\n`);
    } else {
      ...
      if (usersSummary.length > 0) {
        ...
        if (verifyResult.rows[0]) {
          ...
          console.log(`${firstUser.username} now has ${newCoins} total coins (awarded ${firstUser.coinsAwarded})`);
        }
        
        // Check wallet balance too
        const walletResult = await db.execute(sql`
          SELECT balance FROM user_wallet WHERE user_id = ${firstUser.userId}
        `);
        
        if (walletResult.rows[0]) {
          console.log(`Wallet balance: ${(walletResult.rows[0] as any).balance} coins`);
        }
      }
    }
